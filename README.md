...
Example fermi.txt file attached to show format of the file:
[fermi.txt](https://github.com/user-attachments/files/17825914/fermi.txt)
...
**Documentation**
- Add Setup Instructions: Simplify the README by including a step-by-step quickstart guide.
- Database Diagram: Include a basic visual representation of the database schema (use a simple drawing or online tool) in the README.
- Troubleshooting Guide: Add a section on "Known Issues" and how to troubleshoot common errors.
- Developer Guide: Add a CONTRIBUTING.md file with guidelines for setting up the development environment, coding standards, and submitting pull requests.

**App**
- Basic Authentication: Add a simple username/password authentication for accessing the Flask app.
- Mobile-Friendly Design: Make the index.html responsive for better usability on mobile devices.
- Data Export: Allow users to export query results as CSV, JSON, or Excel files for external analysis. (e.g: Add a "Download Results as CSV" button on the search results page.)
- Default Homepage: Display basic project stats (e.g., total jobs processed, metadata table contents) on the homepage when no search query is submitted.

**Additional**
- Preconfigured Setup Scripts: Provide shell scripts for setting up the local development environment, installing dependencies, and initializing the database.
- Better Slack Notifications: Add more detail to Slack messages, such as the number of tests passed/failed or the deployment URL/branch.
